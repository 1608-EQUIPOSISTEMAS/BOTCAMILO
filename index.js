...
*La inversión incluye el CERTIFICADO* 📚`;
        ...
        if (texto.includes("hola") || texto.includes("info") || texto.includes("información") || texto.includes("facilitar")  || texto.includes("quiero")) {
            const resultados = encontrarPrograma(textoOriginal, programasData);

            if (resultados.length === 1) {
                const p = resultados[0];
                let keywordUsed = texto.includes("hola") ? 'hola' : texto.includes("info") ? 'info' : texto.includes("estoy") ? 'estoy' : texto.includes("quiero") ? 'quiero' : texto.includes("facilitar") ? 'facilitar' : null;
